...
class tempDriver{
		...
		private static $nativeArr = array();
		public static function renderTempFromFile($fileName, $mixArgumentArray, $nNativeArr = array(),  $nDebug = false){
			self::$debug = $nDebug;
			
			self::$nativeArr = $nNativeArr;//Native arr checks for fields we dont want displayed
			//$fileName = "templates/firstTemplate.txt";
			$handle = fopen($fileName, "r");
			$template = fread($handle, filesize($fileName));
		
			return tempDriver::parseTemplate($template, $mixArgumentArray);
		}
		...
		public static function hightLightCodeFromFile($fileName){
			...
			$handle = fopen($fileName, "r");
			$template = fread($handle, filesize($fileName));
			return tempDriver::highlightCode($template);
		}
		...
		public static function capatilize($str){
            
			$firstLetter = substr($str, 0, 1);
			$restOStr = substr($str, 1);
			$str = sprintf("%s%s", strtoupper($firstLetter), $restOStr);
            
			return $str;
		}
		public static function isNativeField($fieldName){
			return in_array($fieldName, self::$nativeArr);
